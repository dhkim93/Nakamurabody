...
public class EstimateNewCarController {
	...
	@RequestMapping("searchTabname.ajax")
	public ModelAndView searchTabname(@ModelAttribute("estimateNewCarVO") EstimateNewCarVO estimateNewCarVO,
			Model model) throws Exception {

		ModelAndView mav = new ModelAndView("jsonView");
		List<EstimateNewCarVO> resultList = estimateNewCarSelectSerivce.searchTabname(estimateNewCarVO);
		mav.addObject("searchTabname", resultList);
		return mav;
	}
