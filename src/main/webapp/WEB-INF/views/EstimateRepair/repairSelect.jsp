...
<script type="text/javascript">
	...
	$(function() {
		...
		jqgridTrans.jqGrid({
			datatype : "local",
			colNames : [ "見積番号", "見積日", "車輌タイプ", "工賃金額", "見積担当者", "顧客名" , "customerid"], // colname
			colModel : [ // 
			{
				name : 'estimatenumber'
			}, {
	...
	function searchBodyDetailCombo(sParam){
		...
		$.ajax({
			type : "POST",
			url : "<c:url value='/searchCarTypeCombo.ajax'/>",
			data : {
				"bodytype" : sParam,
			},
			dataType : "json",
	...
	function searchList() {
		...
		var vehicletype = $('#vehicletype').val(); 											//ボデータイプ
		...
		$.ajax({
			type : "POST",
			url : "<c:url value='/selectRepairCar.ajax'/>",
			data : {
...
<body>
	...
	<!-- 修理見積情報選択 -->
	<div class = "repairselect" id ="repairselect">
		<div id="middle">
			<div id="sidebar1400">
				<fieldset>
					<form method=post id="newCarSelectForm" name="newCarSelectForm" action="/popEstimateNewCarDetail.do">
						<table width=100% border=0 cellspacing=0>
							...
							<tr>
								<td width=120 bgcolor=skyblue>車輌タイプ</td>
								<td></td>
								<td>
									<select id="vehicletype" name="vehicletype" onchange="javascript:searchBodyDetailCombo(this.value); return false; ">
